import Base: ==, copy, vec, zero, similar, show
...
(==)(m1::T, m2::T) where T <: Model = all(p1 == p2 for (p1, p2) in zip(allfields(m1), allfields(m2)))
(==)(m1::T1, m2::T2) where {T1 <: Model, T2 <: Model} = false
...
dot(m1::T, m2::T) where T <: Model = _dot(m1, m2)

show(io::IO, m::T) where T <: Model = print(io, T)
